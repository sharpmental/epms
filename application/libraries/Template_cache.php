...
/*		$str = preg_replace("/\{pc:(\w+)\s+([^}]+)\}/ie", "self::pc_tag('$1','$2', '$0')", $str); */
		...
		$str = preg_replace_callback("/\{pc:(\w+)\s+([^}]+)\}/ie",
				function ($match) {
					return self::pc_tag($match[1],$match[2], $match[0]);
				}
				, $str);
		
/*		$str = preg_replace("/\{\/pc\}/ie", "self::end_pc_tag()", $str); */ 
		$str = preg_replace_callback("/\{\/pc\}/ie", 
				function ($match){
					return self::end_pc_tag(); 
					}, $str);
