...
class User extends MY_Admin_Controller {
	...
	function __construct() {
		...
		$this->method_config ['upload'] = array (
				'thumb' => array (
						'upload_size' => 1024,
						'upload_file_type' => 'jpg|png|gif',
						'upload_path' => 'uploadfile/user',
						'upload_url' => '/uploadfile/user/' 
				) 
		);
	}
	...
	function delete() {
		if (isset ( $_POST )) {
			...
			$where = $this->Member_model->to_sqls ( $pidarr, '', 'operator_id' );
	...
	function lock() {
		if (isset ( $_POST )) {
			$pidarr = isset ( $_POST ['pid'] ) ? $_POST ['pid'] : $this->showmessage ( '无效参数', HTTP_REFERER );
			$where = $this->Member_model->to_sqls ( $pidarr, '', 'operator_od' );
	...
	function edit($id = 0) {
		...
		$data_info = $this->Member_model->get_one ( array (
				'operator_id' => $id 
		) );
		...
		if ($this->input->is_ajax_request ()) {
			...
			$password = isset ( $_POST ["password"] ) ? trim ( safe_replace ( $_POST ["password"] ) ) : exit ( json_encode ( array (
					'status' => false,
					'tips' => '密码不能为空' 
			) ) );
			$repassword = isset ( $_POST ["repassword"] ) ? trim ( safe_replace ( $_POST ["repassword"] ) ) : exit ( json_encode ( array (
					'status' => false,
					'tips' => '密码不能为空' 
			) ) );
			...
			$group_id = isset ( $_POST ["group_id"] ) ? intval ( $_POST ["group_id"] ) : exit ( json_encode ( array (
					'status' => false,
					'tips' => '用户组不能为空' 
			) ) );
			if ($group_id == 0) {
			...
			$fullname = isset ( $_POST ["fullname"] ) ? trim ( safe_replace ( $_POST ["fullname"] ) ) : exit ( json_encode ( array (
					'status' => false,
					'tips' => '全名不能为空' 
			) ) );
	...
	function add() {
		...
		if ($this->input->is_ajax_request ()) {
			// 接收POST参数
			$username = isset ( $_POST ["username"] ) ? trim ( safe_replace ( $_POST ["username"] ) ) : exit ( json_encode ( array (
					'status' => false,
					'tips' => '用户名不能为空' 
			) ) );
			if ($username == '')
			...
			$new_id = $this->Member_model->insert ( array (
					'operator_name' => $username,
					'operator_pwd' => $password,
					'operator_role' => $group_id,
					'operator_displayname' => $fullname,
			...
			if ($new_id) {
				// new admin user shall realated with all projects
				if ($group_id == 1) {
					$this->load->model ( array (
							"Project_user_model",
							"Project_model" 
			...
			$this->view ( 'edit', array (
					'is_edit' => false,
					'require_js' => true,
					'data_info' => $this->Member_model->default_info () 
			) );
		}
	}
	 ...
	 *        	HTML控件ID
	 * @param string $callbackJSfunction
	 *        	是否返回函数
	 * @return void
	 */
		...
		if (isset ( $this->method_config ['upload'] [$fieldName] )) {
			if (isset ( $_POST ['dosubmit'] )) {
				...
				$this->view ( 'upload', array (
						...
						'upload_url' => $this->method_config ['upload'] [$fieldName] ['upload_url'],
						'is_image' => $isImage,
						'hidden_menu' => true 
				) );
			}
