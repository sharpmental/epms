...
class Slop extends MY_Admin_Controller {
	...
	function index() {
		...
		foreach ( $r as $k => $v ) {
			...
			if (isset ( $p ) && $p) {
				// add action button
				$modify_link = base_url ( $this->page_data ['folder_name'] . '/slop/modify_slop/' . $v ['slop_id'] );
...
// 					"decompose_picture_path" => isset ( $r [5] ['file_name'] ) ? $pic_path . '/' . $r [5] ['file_name'] : "",
			...
			if (! isset ( $r ) || ! $r) {
				$this->show_error ( "上传文件成功。修改文件路径发生错误。" );
			}
		}
...
// 					"decompose_picture_path" => isset ( $r [5] ['file_name'] ) ? $pic_path . '/' . $r [5] ['file_name'] : "",
			...
			if (! isset ( $r ) || ! $r) {
				$this->show_error ( "上传文件成。修改文件路径时发生错误。" );
			}
		}
